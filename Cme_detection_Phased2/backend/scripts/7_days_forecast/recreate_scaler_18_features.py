...
print(f"All feature columns ({len(feature_cols)}): {feature_cols}")

# Model expects 18 features - exclude Alpha_Proton_Ratio (often has missing data)
# This brings us from 19 to 18 features
features_18 = [col for col in feature_cols if col != 'Alpha_Proton_Ratio']

print(f"\nFeatures for model (18): {features_18}")
print(f"Excluded: Alpha_Proton_Ratio")

# Prepare data for scaling (18 features)
X = train_data[features_18].fillna(0).values
print(f"\nData shape: {X.shape}")

# Fit scaler on 18 features
print("\nFitting StandardScaler on 18 features...")
...
print("\n" + "="*70)
print("✅ SCALER RECREATED SUCCESSFULLY (18 FEATURES - MODEL COMPATIBLE)")
print("="*70)
